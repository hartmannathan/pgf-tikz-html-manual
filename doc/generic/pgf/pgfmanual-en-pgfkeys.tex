...
\begin{command}{\pgfkeysunexpandedvalueof\marg{full key}}
    This is the same as |\pgfkeysvalueof|, but the result will be wrapped
    inside |\unexpanded|.
\end{command}

\begin{command}{\pgfkeysdetokenizedvalueof\marg{full key}}
    This is the same as |\pgfkeysvalueof|, but the result will be wrapped
    inside |\detokenize|.
\end{command}
...
\begin{command}{\pgfkeysifempty\marg{full key}\marg{true code}\marg{false code}}
    Tests whether the value stored in \meta{full key} is empty. If it is
    empty, \meta{true code} is executed, otherwise \meta{false code} is
    executed.

    The command is intended for keys that store values, as created for
    instance by the handler |/.initial| or by |\pgfkeyssetvalue|.
    %
\begin{codeexample}[]
\pgfkeys{/my family/label/.initial=}

\pgfkeysifempty{/my family/label}
  {There is no label.}
  {The label is `\pgfkeysvalueof{/my family/label}'.}

\pgfkeys{/my family/label=Hello}
...
\end{codeexample}
    %
    \emph{Note:} If the key passed to this macro does not exist, the behavior is
    \emph{undefined}.  Currently this macro uses |\pgfkeysdetokenizedvalueof|
    internally which expands to an alias of the |\relax| primitive when the key does
    not exist---therefore \emph{a non-existent key is considered non-empty}.
    However, these semantics could change in the future and must not be relied upon.
    To be safe, check |\pgfkeysifdefined| first.
\end{command}
